...
const ServiceManagement = () => {
  ...
  // ── Store: Categories (chỉ lấy type SERVICE) ──
  ...
  // ── Handlers: Save (Create / Update) ──
  const handleSaveService = async (formData) => {
    const isEdit = !!selectedService;

    if (isEdit) {
      const res = await updateService(selectedService.id, formData);
      if (res?.success) {
        showToast('Cập nhật gói dịch vụ thành công!', 'success');
        setIsModalOpen(false);
        // Giữ nguyên trang hiện tại để kiểm tra dòng vừa sửa
        fetchServices({
          page:       currentPage,
          pageSize:   PAGE_SIZE,
          keyword:    searchTerm.trim() || undefined,
      ...
      if (res?.success) {
        ...
        setIsModalOpen(false);
        setCurrentPage(1); // Về trang 1 để thấy bản ghi mới nhất
      } else {
  ...
  const executeDelete = async () => {
    ...
    if (res?.success) {
      ...
      // Nếu xóa item cuối của trang > 1 thì lùi về trang trước
                  ...
                  <tr key={service.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        {service.serviceImages?.find((img) => img.isThumbnail)?.imageUrl && (
                          <img
                            src={service.serviceImages.find((img) => img.isThumbnail).imageUrl}
                            alt={service.name}
                        ...
                        <div>
                          ...
                          {service.averageRating > 0 && (
                            <div className="flex items-center gap-1 mt-0.5">
                              ...
                              <span className="text-[11px] font-semibold text-amber-600">{service.averageRating}</span>
                              <span className="text-[11px] text-gray-400">({service.totalReviews?.toLocaleString()} đánh giá)</span>
                            </div>
                          )}
                        </div>
                      </div>
