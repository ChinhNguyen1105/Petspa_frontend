...
import { ArrowDownLeft, ArrowUpRight, Sliders, FileText, Package, Search } from "lucide-react";
...
const InventoryTransactionAdmin = ({ onClose, onSubmit, mode = 'CREATE' }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (type === "EXPORT" && currentProduct && Number(quantity) > currentProduct.stockQuantity) {
      alert(`Số lượng xuất vượt quá số lượng tồn kho hiện tại (${currentProduct.stockQuantity})!`);
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-2">
        <label className="text-[11px] font-black uppercase text-gray-400 tracking-wider">Loại giao dịch</label>
        <div className="grid grid-cols-3 gap-2 bg-gray-100 p-1 rounded-xl">
          <button
            ...
            className={`py-2 px-3 text-xs font-black rounded-lg transition-all flex items-center justify-center gap-1 ${
              type === "IMPORT" ? "bg-emerald-500 text-white shadow-sm" : "text-gray-500 hover:text-slate-800"
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-3 text-xs font-black rounded-lg transition-all flex items-center justify-center gap-1 ${
              type === "EXPORT" ? "bg-blue-500 text-white shadow-sm" : "text-gray-500 hover:text-slate-800"
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-3 text-xs font-black rounded-lg transition-all flex items-center justify-center gap-1 ${
              type === "ADJUST" ? "bg-amber-500 text-white shadow-sm" : "text-gray-500 hover:text-slate-800"
            }`}
          >
      ...
      <div className="space-y-1.5">
        ...
        >
          {loading ? (
            ...
            <>
              <option value="">-- Chọn sản phẩm biến động ({filteredProducts.length}) --</option>
              {filteredProducts.map((prod) => (
                <option key={prod.id} value={prod.id}>
                  {/* Đồng bộ: Dùng prod.name và prod.stockQuantity */}
                  [{prod.id}] {prod.name} (Tồn hiện tại: {prod.stockQuantity || 0})
                </option>
              ))}
            </>
          )}
      ...
      <div className="space-y-1.5">
        <label className="text-[11px] font-black uppercase text-gray-400 tracking-wider">
          {type === "ADJUST" ? "Số lượng tồn kho mới sau chốt" : "Số lượng thực hiện"}
        </label>
        <input
          type="number"
          min="1"
          disabled={isViewMode}
          placeholder={type === "ADJUST" ? "Nhập số lượng chốt tồn mới..." : "Nhập số lượng..."}
          value={quantity}
      ...
      {currentProduct && (
        <div className="p-3 bg-slate-50 border border-dashed border-gray-200 rounded-xl grid grid-cols-2 gap-4 text-center">
          <div>
            ...
            <p className="text-sm font-black text-slate-600 font-mono mt-0.5">{currentProduct.stockQuantity || 0}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase">Tồn sau xử lý (Dự kiến)</p>
            <p className={`text-sm font-black font-mono mt-0.5 ${
              previewStock < 0 ? "text-red-500" : type === "IMPORT" ? "text-emerald-600" : type === "EXPORT" ? "text-blue-600" : "text-amber-500"
            }`}>
              {previewStock}
            </p>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
          ...
          <button
            ...
            className={`px-5 py-2 text-white text-xs font-black rounded-xl transition-all shadow-sm ${
              type === "IMPORT" ? "bg-emerald-500 hover:bg-emerald-600" : type === "EXPORT" ? "bg-blue-500 hover:bg-blue-600" : "bg-amber-500 hover:bg-amber-600"
            } active:scale-95`}
          >
            XÁC NHẬN GHI SỔ
          </button>
